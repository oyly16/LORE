...
def vis_first_step_attnmap(attn_map_list,token_ids,target_objects,output_attn_dir,width,height,savename,save_layers=None):
    if save_layers is None:
        save_layers = [range(i*10,i*10+10) for i in range(5)]
        save_layers.append(range(50,57))

    attn_map_list = attn_map_list.to(torch.float32)
    timeidx = 0

    for layers in save_layers:
        for l in layers:
            N_words = len(token_ids)
            save_filename = f'attnmap_{savename}_layer{l:02d}'

            plt.figure(dpi=300)
            plt.suptitle(save_filename, fontsize=10)
            for j, (save_word_ids,_,_) in enumerate(token_ids):
                attnmap = attn_map_list[timeidx, l, :, :, save_word_ids]
                attnmap = torch.mean(attnmap, dim=-1)
                ...
                plt.subplot(1, N_words, j + 1)
                plt.imshow(attnmap[0].cpu().numpy(), cmap="viridis")
                plt.axis('off')
                plt.title(target_objects[j], fontsize=5)

            plt.subplots_adjust(left=0, right=1, top=0.9, bottom=0, wspace=0.05, hspace=0.05)
            plt.savefig(os.path.join(output_attn_dir, save_filename + '.png'))
            plt.clf()
            plt.close()
...
def main(
    ...
    for seed in args.seeds:
        ...
        # save attnmap visualization
        vis_first_step_attnmap(attn_map_list,token_ids,[args.target_object],args.savedir,width,height,args.savename)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--savedir', default="outputs_attnmap", type=str,
                        help='save dir')
